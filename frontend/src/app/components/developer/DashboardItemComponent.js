...
const DashboardItemComponent = ({ icon, number, text }) => {
  return (
    <div className="w-[15rem] h-[10rem] border-solid p-2 rounded-xl text-center bg-colorFour shadow-xl flex flex-col justify-center">
      <span className="flex justify-center text-4xl text-tertiary">{icon}</span>
      <div>
        {number === 0 ? <></> : <p className="text-xl">{number}</p>}
